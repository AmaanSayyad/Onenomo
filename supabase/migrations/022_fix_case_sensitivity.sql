-- Fix case sensitivity issues in stored procedures
-- All address comparisons should be case-insensitive using LOWER()
...
BEGIN
    ...
    -- Lock the row for update with currency filter (case-insensitive address comparison)
...
BEGIN
    ...
    VALUES (LOWER(p_user_address), p_currency, 'bet_won', p_payout_amount, v_current_balance, v_new_balance, p_bet_id);
