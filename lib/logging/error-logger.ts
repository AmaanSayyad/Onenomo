...
function getUserAddress(): string | undefined {
  ...
  try {
    ...
    const sessionData = localStorage.getItem('bynomo_sui_wallet_session');
    if (sessionData) {
...
function getSessionId(): string {
  ...
  try {
    let sessionId = sessionStorage.getItem('bynomo_session_id');
    if (!sessionId) {
...
export function clearStoredErrorLogs(): void {
  ...
  try {
    localStorage.removeItem('bynomo_error_logs');
